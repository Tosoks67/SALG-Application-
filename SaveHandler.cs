...
namespace SALG__Application_
{
    public static class SaveHandler
    {
        ...
        /// backs up corrupt JSON files and returns a default object if nothing exists.
        /// </summary>
        public static UserData LoadUserData()
        {
            ...
            if (File.Exists(DataPath))
            {
                try
                {
                    ...
                    UserData? data = JsonSerializer.Deserialize<UserData>(json, JsonOptions);
            ...
            if (File.Exists("data"))
            {
                try
                {
                    string legacy = File.ReadAllText("data");
                    UserData? migrated = MigrateLegacyData(legacy);
                    if (migrated != null)
                    {
        ...
        /// </summary>
        public static void SaveUserData(UserData data, string path = "default")
        {
            path = path == "default" ? DataPath : path;
            Directory.CreateDirectory(AppFolder);
        ...
        public static Prefs LoadPreferences()
        {
            ...
            if (File.Exists(PrefsPath))
            {
                try
                {
                    ...
                    Prefs? prefs = JsonSerializer.Deserialize<Prefs>(json, JsonOptions);
            ...
            if (File.Exists("misc"))
            {
                try
                {
                    string legacy = File.ReadAllText("misc");
                    Prefs? migrated = MigrateLegacyMisc(legacy);
                    if (migrated != null)
                    {
        ...
        public static string LoadNotes()
        {
            ...
            {
                string[] notes = File.ReadAllLines(NotesPath);
            ...
            if (File.Exists("notes"))
            {
                try
                {
                    string[] legacy = File.ReadAllLines("notes");
                    string current = legacy.Length >= 1 ? legacy[0] : "";
        ...
        private static void BackupCorruptFile(string path, string reason)
        {
            try
            {
                ...
                string dest = Path.Combine(BackupFolder, $"{name}.corrupt.{safeReason}.{stamp}.bak");
                File.Move(path, dest);
            }
            catch
            {
        ...
        private static void BackupLegacyFile(string path)
        {
            try
            {
                ...
                string stamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
                string name = Path.GetFileName(path);
                string dest = Path.Combine(BackupFolder, $"{name}.migrated.{stamp}.bak");
                File.Move(path, dest);
            }
            catch
            {
        ...
        /// otherwise null.
        /// Syntax: username|rank|qDone|tTime|cQuota|show qDone (Y/N)
        /// Adjust parsing logic to match your actual legacy format precisely.
        /// </summary>
        private static UserData? MigrateLegacyData(string legacyContent)
        {
            ...
            string[] parts = legacyContent.Split('|');
            ...
            try
            {
                ...
                if (int.TryParse(parts[2].Trim(), out int qDone))
                    ud.QuotaDone = qDone;
                else
                    ud.QuotaDone = 0;

                if (int.TryParse(parts[3].Trim(), out int tTime))
                    ud.TotalTime = tTime;
                else
                    ud.TotalTime = 0;

                if (int.TryParse(parts[4].Trim(), out int cQuota))
        ...
        private static Prefs? MigrateLegacyMisc(string legacyContent)
        {
            ...
            {
                Prefs prefs = new();
                prefs.DarkMode = parts[0].Trim().ToUpper() == "Y";
                prefs.TRT = parts[1].Trim().ToUpper() == "Y";
